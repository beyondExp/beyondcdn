...
namespace Beyond\BeyondCDN\Tests;
...
use PlatformCommunity\Flysystem\BunnyCDN\BeyondCDNAdapter;
use PlatformCommunity\Flysystem\BunnyCDN\BeyondCDNClient;
use PlatformCommunity\Flysystem\BunnyCDN\BeyondCDNRegion;
...
class FlysystemTestSuite extends TestCase
{
     ...
     * @return BeyondCDNAdapter
     */
    protected static function createFilesystemAdapter(string $prefixPath = ''): BeyondCDNAdapter
    {
        return new BeyondCDNAdapter(static::bunnyCDNClient(), self::PULLZONE_URL, $prefixPath);
    }

    /**
     * @return BeyondCDNAdapter
     */
    public static function createPrefixFilesystemAdapter(): BeyondCDNAdapter
    {
        return static::createFilesystemAdapter(static::$prefixPath);
    }

    private static function bunnyCDNClient(): BeyondCDNClient
    {
        if (static::$bunnyCDNClient instanceof BeyondCDNClient) {
            return static::$bunnyCDNClient;
        }

        if (isset($_SERVER['STORAGEZONE'], $_SERVER['APIKEY'])) {
            static::$bunnyCDNClient = new BeyondCDNClient($_SERVER['STORAGEZONE'], $_SERVER['APIKEY']);
    ...
    {
        $adapter = new BeyondCDNAdapter(
            new BeyondCDNClient(self::STORAGE_ZONE, 'api-key')
        );
    ...
    public function test_regression_issue_21()
    {
        ...
        $adapter = new BeyondCDNAdapter($client);
